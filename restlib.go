...
func (t *RestAgent) HttpPostJson(url, payload string, header map[string]string) []map[string]interface{} {
	var jsonStr = []byte(payload)
	var response []map[string]interface{}
	req, err := http.NewRequest("POST", fmt.Sprintf("https://%v:8443/oms1350%v", t.IpAddress, url), bytes.NewBuffer(jsonStr))
	errDealer(err)
